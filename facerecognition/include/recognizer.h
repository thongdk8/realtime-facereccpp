...
class FR_MFN_Deploy {
  ...
  public:
    FR_MFN_Deploy(std::string modelFolder) {
        ...
        // load param
        std::ifstream params_in(modelFolder + "/deploy_param.params",
                                std::ios::binary);
    ...
    cv::Mat forward(cv::Mat inputImageAligned) {
        ...
        cv::Mat tensor =
            cv::dnn::blobFromImage(inputImageAligned, 1.0, cv::Size(112, 112),
...
inline float L2Distance(const cv::Mat &v1, const cv::Mat &v2){
    cv::Mat sub;
